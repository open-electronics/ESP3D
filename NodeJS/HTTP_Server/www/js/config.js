...
function refreshconfig(is_override) {
    ...
    getById('config_loader').style.display = "block";
    getById('config_list_content').style.display = "none";
    getById('config_status').style.display = "none";
    getById('config_refresh_btn').style.display = "none";
...
function config_display_override(display_it) {
    if (display_it) {
        getById('config_override_list_content').style.display = "block";
        getById('config_main_content').style.display = "none";
        getById('config_override_file').checked = true;
    } else {
        getById('config_override_list_content').style.display = "none";
        getById('config_main_content').style.display = "block";
        getById('config_main_file').checked = true;
    }
}
...
function Apply_config_override() {
	...
	//MIRKO
	if (!remove) {  
	  url = esp32_address + "/command?plain=" + encodeURIComponent("M500");
	}else{
	  url = "/command?plain=" + encodeURIComponent("M500");
	}	
...
function Delete_config_override() {
	...
	if (!remove) {
	  url = esp32_address + "/command?plain=" + encodeURIComponent("M502");
	}else{
	  url = "/command?plain=" + encodeURIComponent("M502");
	}
...
function build_HTML_config_list() {
    ...
    if (is_override_config){
		array_len = config_override_List.length;
	}
    ...
    if (content.length > 0) {
        if (target_firmware == "smoothieware") {
            getById('config_main_file_name').innerHTML = config_file_name;
            if (!is_override_config) {
                getById('config_list_data').innerHTML = content;
                getprinterconfig(true);
            } else {
                getById('config_override_data').innerHTML = content;
                if (is_config_override_file()) {
                    getById('config_delete_override').style.display = 'none';
                    getById('config_override_file_name').innerHTML = "Smoothieware";
                } else {
                    getById('config_override_file_name').innerHTML = "/sd/config-override";
                    getById('config_delete_override').style.display = 'block';
                }
            }
        } else {
            getById('config_list_data').innerHTML = content;
    ...
    getById('config_loader').style.display = "none";
    getById('config_list_content').style.display = "block";
    getById('config_status').style.display = "none";
    getById('config_refresh_btn').style.display = "block";
}
...
function process_config_answer(response_text) {
    ...
    if (tlines.length <= 3) {
        if ((target_firmware == "smoothieware") && (commandtxt != "cat /sd/config.txt")) {
            ...
            if ((target_firmware == "smoothieware")) getById('config_status').innerHTML = translate_text_item("File config / config.txt not found!");
            else getById('config_status').innerHTML = translate_text_item("Cannot get EEPROM content!");
            result = false;
        }
    } else {
...
function create_config_entry(sentry, vindex) {
    ...
    if ((target_firmware == "marlin") || (target_firmware == "marlinkimbra")) {
		if (sentry.startsWith("Config:  ")) {
		   ssentry = sentry.replace("Config:", "");
		}else{
		   ssentry = sentry.replace("Config:", "#");
		}
	}
    ...
    if ((target_firmware == "marlin") || (target_firmware == "marlinkimbra") || (target_firmware == "marlin-embedded")) {
		if (sentry.startsWith("echo:  ")){ 
			ssentry = sentry.replace("echo:", "");
		}else{
			ssentry = sentry.replace("echo:", "#");
	    }
	}
...
function is_config_entry(sline) {
    ...
    if ((target_firmware == "marlin") || (target_firmware == "marlinkimbra") || (target_firmware == "marlin-embedded")) {
        if (sline.startsWith("Config:") || sline.startsWith("echo:") || sline.startsWith("\t") || sline.startsWith("  ")){
		  return true;
        }else{  
		  return false;
		}
...
function get_config_help(sline) {
  ...
  if (is_override_config) {
    ...
    if (pos > -1) {
      return sline.slice(pos + 1, sline.length);
    }else{
      return "";
    }
  }
...
function config_revert_to_default(index, is_override) {
    ...
    console.log();
    getById('config_' + prefix + index).value = item.defaultvalue;
    getById('btn_config_' + prefix + index).className = "btn btn-default";
    getById('status_config_' + prefix + index).className = "form-group has-feedback";
    getById('icon_config_' + prefix + index).innerHTML = "";
}
...
function configGetvalue(index, is_override) {
    ...
    value = getById('config_' + prefix + index).value.trim();
    if (value == item.defaultvalue) return;
    //check validity of value
    var isvalid = config_check_value(value, index, is_override);
    //if not valid show error
    if (!isvalid) {
        getById('btn_config_' + prefix + index).className = "btn btn-danger";
        getById('icon_config_' + prefix + index).className = "form-control-feedback has-error ico_feedback";
        getById('icon_config_' + prefix + index).innerHTML = get_icon_svg("remove");
        getById('status_config_' + prefix + index).className = "form-group has-feedback has-error";
        ...
        getById('btn_config_' + prefix + index).className = "btn btn-success";
        getById('icon_config_' + prefix + index).className = "form-control-feedback has-success ico_feedback";
        getById('icon_config_' + prefix + index).innerHTML = get_icon_svg("ok");
        getById('status_config_' + prefix + index).className = "form-group has-feedback has-success";
...
function config_checkchange(index, is_override) {
    ...
    var val = getById('config_' + prefix + index).value.trim();
    //console.log("value: " + val);
    if (item.defaultvalue == val) {
        getById('btn_config_' + prefix + index).className = "btn btn-default";
        getById('icon_config_' + prefix + index).className = "form-control-feedback";
        getById('icon_config_' + prefix + index).innerHTML = "";
        getById('status_config_' + prefix + index).className = "form-group has-feedback";
    } else if (config_check_value(val, index, is_override)) {
        getById('status_config_' + prefix + index).className = "form-group has-feedback has-warning";
        getById('btn_config_' + prefix + index).className = "btn btn-warning";
        getById('icon_config_' + prefix + index).className = "form-control-feedback has-warning ico_feedback";
        getById('icon_config_' + prefix + index).innerHTML = get_icon_svg("warning-sign");
...
function setESPconfigfailed(error_code, response) {
    ...
    getById('btn_config_' + prefix + config_lastindex).className = "btn btn-danger";
    getById('icon_config_' + prefix + config_lastindex).className = "form-control-feedback has-error ico_feedback";
    getById('icon_config_' + prefix + config_lastindex).innerHTML = get_icon_svg("remove");
    getById('status_config_' + prefix + config_lastindex).className = "form-group has-feedback has-error";
}
...
function getESPconfigfailed(error_code, response) {
    console.log("Error " + error_code + " :" + response);
    getById('config_loader').style.display = "none";
    getById('config_status').style.display = "block";
    getById('config_status').innerHTML = translate_text_item("Failed:") + error_code + " " + response;
    getById('config_refresh_btn').style.display = "block";
}
