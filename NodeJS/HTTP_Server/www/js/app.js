...
var camera_address = "";
var camera_refesh_interval_id;


try {
	
var embed = getById("svgObject");		

embed.addEventListener('load', function(){	

	// MIRKO:
	//https://stackoverflow.com/questions/337293/how-to-check-if-an-embedded-svg-document-is-loaded-in-an-html-page
	//https://www.html.it/articoli/il-problema-di-windowonload/
...
window.onload = function() {
    ...
    getById('loadingmsg').style.display = 'none';
    ...
    if (browser_is("IE")){
		getById('control-body').className= "panel-body";
		getById('extruder-body').className= "panel-body panel-height";
		getById('command-body').className= "panel-body";
		getById('file-body').className= "panel-body panel-height panel-max-height panel-scroll";
	}
...
function ontoggleLock(forcevalue) {
    if (typeof forcevalue != 'undefined') getById('lock_UI').checked = forcevalue;
    if (getById('lock_UI').checked) {
        getById('lock_UI_btn_txt').innerHTML = translate_text_item("Unlock interface");
        disable_items(getById('maintab'), true);
        disable_items(getById('configtab'), true);
        getById('progress_btn').disabled = false;
        getById('clear_monitor_btn').disabled = false;
        getById('monitor_enable_verbose_mode').disabled = false;
        getById('monitor_enable_autoscroll').disabled = false;
        getById('settings_update_fw_btn').disabled = true;
        getById('settings_restart_btn').disabled = true;
        disable_items(getById('JogUI'), false);
        getById('JogUI').style.pointerEvents = 'none';
    } else {
        getById('lock_UI_btn_txt').innerHTML = translate_text_item("Lock interface");
        disable_items(getById('maintab'), false);
        disable_items(getById('configtab'), false);
        getById('settings_update_fw_btn').disabled = false;
        getById('settings_restart_btn').disabled = false;
        getById('JogUI').style.pointerEvents = 'auto';
    }
}
...
function Handle_DHT(data) {
    ...
    if (typeof getById('DHT_humidity') != "undefined") {
       getById('DHT_humidity').innerHTML = parseFloat(tdata[1]).toFixed(2).toString() + "%";
	}
	
    var temps = temp.toFixed(2).toString() + "&deg;";
    
	if(convertDHT2Fahrenheit) {
	   temps+="F";
    }else{ 
	   temps+="C";
	}
	
    getById('DHT_temperature').innerHTML = temps;
}
...
function display_boot_progress(step) {
    ...
    getById('load_prg').value = Math.round((current_boot_steps * 100) / total_boot_steps);
}
...
function Disable_interface(lostconnection) {
    ...
    if (typeof lostconnection  != "undefined"){
		lostcon= lostconnection;
	}
...
function update_UI_firmware_target() {
    var fwName;
    initpreferences();
    getById('control_x_position_label').innerHTML = "X";
    getById('control_y_position_label').innerHTML = "Y";
    getById('control_z_position_label').innerHTML = "Z";
    getById('config_smoothie_nav').style.display = 'none';
    showAxiscontrols();
    if (target_firmware == "repetier") {
        fwName = "Repetier";
        getById('configtablink').style.display = 'block';
        getById('auto_check_control').style.display = 'flex';
        getById('motor_off_control').style.display = 'table-row';
        getById('progress_btn').style.display = 'table-row';
        getById('abort_btn').style.display = 'table-row';
        getById('grblPanel').style.display = 'none';
        getById('zero_xyz_btn').style.display = 'none';
        getById('zero_x_btn').style.display = 'none';
        getById('zero_y_btn').style.display = 'none';
        getById('zero_z_btn').style.display = 'none';
        getById('control_xm_position_row').style.display = 'none';
        getById('control_ym_position_row').style.display = 'none';
        getById('control_zm_position_row').style.display = 'none';
        ...
        fwName = "Smoothieware";
        getById('configtablink').style.display = 'block';
        getById('config_smoothie_nav').style.display = 'block';
        getById('auto_check_control').style.display = 'flex';
        getById('motor_off_control').style.display = 'table-row';
        getById('progress_btn').style.display = 'table-row';
        getById('abort_btn').style.display = 'table-row';
        ...
        last_grbl_pos = "";
        getById('configtablink').style.display = 'block';
        getById('auto_check_control').style.display = 'none';
        getById('progress_btn').style.display = 'none';
        getById('abort_btn').style.display = 'none';
        getById('motor_off_control').style.display = 'none';
        getById('tab_title_configuration').innerHTML = "<span translate>GRBL configuration</span>";
        getById('tab_printer_configuration').innerHTML = "<span translate>GRBL</span>";
        getById('files_input_file').accept = " .g, .gco, .gcode, .txt, .ncc, .G, .GCO, .GCODE, .TXT, .NC";
        getById('zero_xyz_btn').style.display = 'block';
        getById('zero_x_btn').style.display = 'block';
        getById('zero_y_btn').style.display = 'block';
        if (grblaxis >2) {
            //getById('control_z_position_display').style.display = 'block';
            getById('control_z_position_label').innerHTML = "Zw";
            getById('zero_xyz_btn_txt').innerHTML +="Z";
            grblzerocmd += " Z0";
            
        } else {
            hideAxiscontrols();
            getById('preferences_control_z_velocity_group').style.display = 'none';
        }
        if (grblaxis >3) {
            
            getById('zero_xyz_btn_txt').innerHTML +="A";
            grblzerocmd += " A0";
            build_axis_selection();
            getById('preferences_control_a_velocity_group').style.display = 'block';
            getById('positions_labels2').style.display = 'inline-grid';
            getById('control_a_position_display').style.display = 'block';
        } 
        if (grblaxis >4) {
            getById('control_b_position_display').style.display = 'block';
            getById('zero_xyz_btn_txt').innerHTML +="B";
            grblzerocmd += " B0";
            getById('preferences_control_b_velocity_group').style.display = 'block';
        } 
        if (grblaxis >5) {
            getById('control_c_position_display').style.display = 'block';
            getById('zero_xyz_btn_txt').innerHTML +="C";
            getById('preferences_control_c_velocity_group').style.display = 'block';
            
        } else {
            getById('control_c_position_display').style.display = 'none';
        }
        getById('grblPanel').style.display = 'flex';
        getById('FW_github').href = 'https://github.com/bdring/Grbl_Esp32';
        getById('settings_filters').style.display = 'none';
        getById('control_x_position_label').innerHTML = "Xw";
        getById('control_y_position_label').innerHTML = "Yw";
        getById('control_z_position_label').innerHTML="Zw";  
        ...
        getById('zero_z_btn').style.display = 'none';
        getById('grblPanel').style.display = 'none';
        getById('FW_github').href = 'https://github.com/MarlinFirmware/Marlin';
        getById('settings_filters').style.display = 'none';
        getById('control_xm_position_row').style.display = 'none';
        getById('control_ym_position_row').style.display = 'none';
        getById('control_zm_position_row').style.display = 'none';
        ...
        getById('zero_x_btn').style.display = 'block';
        getById('zero_y_btn').style.display = 'block';
        getById('zero_z_btn').style.display = 'block';
        getById('grblPanel').style.display = 'flex';
        getById('control_x_position_label').innerHTML = "Xw";
        getById('control_y_position_label').innerHTML = "Yw";
        getById('control_z_position_label').innerHTML = "Zw";
        getById('control_xm_position_row').style.display = 'table-row';
        getById('control_ym_position_row').style.display = 'table-row';
        getById('control_zm_position_row').style.display = 'table-row';
    } else {
        fwName = "Unknown";
        getById('configtablink').style.display = 'none';
    }
    ...
    if ( typeof (getById('fwName')) != "undefined" && getById('fwName') != null){
		getById('fwName').innerHTML = fwName;
	}
	
	
    if ( typeof (getById('showSDused')) != "undefined" && getById('showSDused')!= null && direct_sd){
		getById('showSDused').innerHTML="<svg width='1.3em' height='1.2em' viewBox='0 0 1300 1200'><g transform='translate(50,1200) scale(1, -1)'><path  fill='#777777' d='M200 1100h700q124 0 212 -88t88 -212v-500q0 -124 -88 -212t-212 -88h-700q-124 0 -212 88t-88 212v500q0 124 88 212t212 88zM100 900v-700h900v700h-900zM500 700h-200v-100h200v-300h-300v100h200v100h-200v300h300v-100zM900 700v-300l-100 -100h-200v500h200z M700 700v-300h100v300h-100z' /></g></svg>";		
	}
...
function initUI() {
    ...
    if ( getById('UI_VERSION') != null && typeof getById('UI_VERSION') != "undefined" ){
		getById('UI_VERSION').innerHTML=web_ui_version;
	}
		
	
    if (getById('FW_VERSION')!= null && typeof getById('FW_VERSION') != "undefined"){
		getById('FW_VERSION').innerHTML=fw_version;
	}
    ...
    if ((target_firmware == "grbl-embedded") || (target_firmware == "grbl-embedded")) {
        if (typeof getById("grblcontroltablink") !== 'undefined') {
