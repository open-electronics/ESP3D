...
bool CONFIG::reset_config(){
    ...
    if (!CONFIG::write_byte (ESP_SONOFF_TYPE, DEFAULT_SONOFF_TYPE) ) {
        return false;
    }
	
	if (!CONFIG::write_buffer (ESP_SONOFF_IP1, DEFAULT_IP_VALUE, IP_LENGTH) ) {
        return false;
    }
	
	if (!CONFIG::write_buffer (ESP_SONOFF_IP2, DEFAULT_IP_VALUE, IP_LENGTH) ) {
        return false;
    }
#endif
