...
function connectsuccess(response) {
    if (getFWdata(response)) {
        ...
        if (ESP3D_authentication) {
            ...
            getById('menu_authentication').style.display = 'inline';
            logindlg(initUI, true);
        } else {
            getById('menu_authentication').style.display = 'none';
            initUI();
        }
    } else {
...
function connectfailed(errorcode, response) {
    getById('connectbtn').style.display = 'block';
    getById('failed_connect_msg').style.display = 'block';
    getById('connecting_msg').style.display = 'none';
    console.log("Fw identification error " + errorcode + " : " + response);
}

function retryconnect() {
	var url="";
    getById('connectbtn').style.display = 'none';
    getById('failed_connect_msg').style.display = 'none';
    getById('connecting_msg').style.display = 'block';

	//MIRKO
	if (!remove) {
	  url = esp32_address + "/command?plain="+encodeURIComponent("[ESP800]");
	}else{
	  url = "/command?plain="+encodeURIComponent("[ESP800]");
	}
