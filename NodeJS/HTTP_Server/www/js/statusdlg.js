...
function update_btn_status(forcevalue) {
    ...
    if (statuspage == 0) {
        statuspage = 1;
        getById('next_status_btn').innerHTML = get_icon_svg("triangle-right", "1em", "1em")
    } else {
        statuspage = 0;
        getById('next_status_btn').innerHTML = get_icon_svg("triangle-left", "1em", "1em")
    }
}

function statussuccess(response) {
    getById('refreshstatusbtn').style.display = 'block';
    getById('status_loader').style.display = 'none';
    var modal = getactiveModal();
...
function statusfailed(errorcode, response) {
    getById('refreshstatusbtn').style.display = 'block';
    getById('status_loader').style.display = 'none';
    getById('status_msg').style.display = 'block';
    console.log("Error " + errorcode + " : " + response);
    getById('status_msg').innerHTML = "Error " + errorcode + " : " + response;
}

function refreshstatus() {
    getById('refreshstatusbtn').style.display = 'none';
    getById('status_loader').style.display = 'block';
    var modal = getactiveModal();
    ...
    var text = modal.element.getElementsByClassName("modal-text")[0];
    text.innerHTML = "";
    getById('status_msg').style.display = 'none';
	var url="";
	...
	//MIRKO
	if (!remove){	
		url = esp32_address + "/command?plain=" + encodeURIComponent("[ESP420]plain");
	}else{
		url = "/command?plain=" + encodeURIComponent("[ESP420]plain");
	}
