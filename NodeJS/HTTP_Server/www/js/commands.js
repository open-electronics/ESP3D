...
function Monitor_output_autoscrollcmd() {
    getById('cmd_content').scrollTop = getById('cmd_content').scrollHeight;
}

function Monitor_check_autoscroll() {
    if (getById('monitor_enable_autoscroll').checked == true) Monitor_output_autoscrollcmd();
}
...
function Monitor_output_Update(message) {
    ...
    var isverbosefilter = getById("monitor_enable_verbose_mode").checked;
    for (var i = 0; i < Monitor_outputLength; i++) {
        //Filter the output  
        if ((Monitor_output[i].trim().toLowerCase() == "ok") && !isverbosefilter) {
			continue; 
		}
		
        if ((Monitor_output[i].trim().toLowerCase() == "wait") && !isverbosefilter) {
			continue;  
		} 
        ...
        if ((m.startsWith("echo:") || m.startsWith("Config:")) && !isverbosefilter){
		  continue;
		}
				
        if (m.startsWith("echo:Unknown command: \"echo\"") ||  (m.startsWith("echo:enqueueing \"*\""))){
		  continue;
		}
			
        output += m;
    }
    getById("cmd_content").innerHTML = output;
    Monitor_check_autoscroll();
}

function SendCustomCommand() {
    var url="";
    var cmd = getById("custom_cmd_txt").value;
...
function CustomCommand_OnKeyUp(event) {
    ...
    if (event.keyCode == 38 || event.keyCode == 40) {
        ...
        if (CustomCommand_history_index >= 0 && CustomCommand_history_index < CustomCommand_history.length) {
            getById("custom_cmd_txt").value = CustomCommand_history[CustomCommand_history_index];
        }
        return false;
    }
    return true;
