...
function hideZcontrols(){
	...
	try{
		...
		var a = getById("svgObject");
		var svgDoc = a.contentDocument;
		
		svgDoc.getElementById('JogBar').style.display = 'none';
		svgDoc.getElementById('HomeZ').style.display = 'none';
		svgDoc.getElementById('CornerZ').style.display = 'block';
		
		getById('control_z_position_display').style.display = 'none';
		getById('control_zm_position_row').style.display = 'none';
		getById('z_velocity_display').style.display = 'none';
		
	}catch (exception){
		console.log ("hideZcontrols:" + exception);
	}
...
function showAxiscontrols(){
	...
	try{		
		...
		svgDoc.getElementById('CornerZ').style.display = 'none';
		svgDoc.getElementById('JogBar').style.display = 'block';
		svgDoc.getElementById('HomeZ').style.display = 'block';
		...
		getById('z_velocity_display').style.display = 'inline';
    }catch (exception){
		console.log ("showZcontrols:" + exception);
	}
...
function loadmacrolist() {
	...
	//MIRKO
	if (!remove) {
	  url = esp32_address + "/macrocfg.json" + "?" + Date.now();
	}else{
	  url = "/macrocfg.json" + "?" + Date.now();
	}
...
function on_autocheck_position(use_value){
    ...
    if (typeof (use_value) !== 'undefined' ){
		getById('autocheck_position').checked = use_value;
	}  
	
    if (getById('autocheck_position').checked) {
       var interval = parseInt(getById('posInterval_check').value);   
	   ...
	   if (!isNaN(interval) && interval > 0 && interval < 100) {  
           ...
           interval_position = setInterval(function(){ get_Position() }, interval * 1000);
            ...
            getById('autocheck_position').checked = false;
            getById('posInterval_check').value = 0;
            if (interval_position != -1 ){
				clearInterval(interval_position);
			}	
            interval_position = -1;
...
function Control_get_position_value(label, result_data) {
    ...
    if (pos1 > -1){
        ...
        if (pos2 > -1){
			...
			result = result_data.substring(pos1);
		} 
    }
    return result.trim();
}
...
function process_Position(response){
    if ((target_firmware == "grbl") || (target_firmware == "grbl-embedded")){
        ...
        getById('control_x_position').innerHTML = Control_get_position_value("X:", response);
        getById('control_y_position').innerHTML = Control_get_position_value("Y:", response);
        getById('control_z_position').innerHTML = Control_get_position_value("Z:", response);
    }
}
...
function SendHomecommand (cmd){
     if (getById('lock_UI').checked )return;
     if ((target_firmware == "grbl-embedded" ) || (target_firmware == "grbl" )) {
        switch(cmd){
        case 'G28': 
            cmd = '$H';
...
function SendJogcommand(cmd, feedrate){
    ...
    if (feedrate == "XYfeedrate") {
        feedratevalue = parseInt(getById('control_xy_velocity').value);
		...
		if (feedratevalue < 1 || isNaN(feedratevalue) || (feedratevalue === null)) {
            ...
            getById('control_xy_velocity').value = preferenceslist[0].xy_feedrate;
            return;
        }
		
    }else{
        feedratevalue = parseInt(getById('control_z_velocity').value);
         if (feedratevalue < 1  || isNaN(feedratevalue) || (feedratevalue === null)) {
            alertdlg (translate_text_item("Out of range"), translate_text_item( "Z Feedrate value must be at least 1 mm/min!"));
            getById('control_z_velocity').value = preferenceslist[0].z_feedrate;
            return;
        }
    }
 ...
 function onXYvelocityChange () {
    var feedratevalue =  parseInt(getById('control_xy_velocity').value);
...
function onZvelocityChange () {
    var feedratevalue =  parseInt(getById('control_z_velocity').value);
...
function control_build_macro_ui(){
    ...
    getById('Macro_list').innerHTML=content;
}
