...
function build_accept(file_filters_list) {
    ...
    getById('files_input_file').accept = accept_txt;
...
function init_files_panel(dorefresh) {
    if (target_firmware == "smoothieware") {
        ...
        getById('files_refresh_primary_sd_btn').innerHTML = primary_sd.substring(0, primary_sd.length - 1);
        getById('files_refresh_secondary_sd_btn').innerHTML = secondary_sd.substring(0, secondary_sd.length - 1);
        
		if (primary_sd.toLowerCase() != "none" ){
			getById('files_refresh_primary_sd_btn').style.display="inline";
		}
       
	    if (secondary_sd.toLowerCase() != "none" ){
			getById('files_refresh_secondary_sd_btn').style.display="inline";
		}
        ...
        if (target_firmware == "???") {
		    ...
		    getById('files_refresh_btn').style.display="inline";
		}
		
        getById('files_refresh_primary_sd_btn').style.display = "none";
        getById('files_refresh_secondary_sd_btn').style.display = "none";
    ...
    if (!((target_firmware == "marlin") || (target_firmware == "???") ||(target_firmware == "grbl"))) getById('files_createdir_btn').style.display = "inline";
    else getById('files_createdir_btn').style.display = "none";
...
function files_set_button_as_filter(isfilter) {
    if (!isfilter) {
        getById('files_filter_glyph').innerHTML = get_icon_svg("filter", "1em", "1em");
    } else {
        getById('files_filter_glyph').innerHTML = get_icon_svg("list-alt", "1em", "1em");
    }
}
...
function files_build_file_line(index) {
    ...
    if ((files_filter_sd_list && entry.isprintable) || (!files_filter_sd_list)) {
		...
		if (entry.isdir == true){ 
		  content +=get_icon_svg("folder-open");
        }else{ 
        ...
        if (entry.isprintable) {
            ...
            if ((target_firmware == "grbl-embedded") || (target_firmware == "grbl")) {
			  ...
			  content +=get_icon_svg("print","1em","1em");
			}
...
function files_create_dir(name) {
    ...
    if (direct_sd && !((target_firmware == "smoothieware") && files_currentPath.startsWith(secondary_sd))) {
		...
		//MIRKO
		if (!remove) {
		  url = esp32_address + "/upload?path="+encodeURIComponent(cmdpath) + "&action=createdir&filename=" + encodeURIComponent(name);
		}else{
		  url = "/upload?path=" + encodeURIComponent(cmdpath) + "&action=createdir&filename=" + encodeURIComponent(name);
		}
		
        getById('files_nav_loader').style.display = "block";
...
function files_delete_file(index) {
    ...
    if (direct_sd && !((target_firmware == "smoothieware") && files_currentPath.startsWith(secondary_sd))) {
		...
		//MIRKO
		if (!remove) {
			url = esp32_address + "/upload?path="+encodeURIComponent(cmdpath)+"&action=";
		}else{
			url = "/upload?path="+encodeURIComponent(cmdpath)+"&action=";
		}
...
function files_proccess_and_update(answer) {
    getById('files_navigation_buttons').style.display = "block";
    if (answer.startsWith("{") && answer.endsWith("}")) {
       ...
       if (answer.length > 0) {
		   files_error_status = answer;
       }else if (files_error_status.length == 0){
...
function files_click_file(index) {
    ...
    if (direct_sd && (!(target_firmware == "smoothieware" && files_currentPath.startsWith(secondary_sd)) || (target_firmware != "smoothieware"))) {
        ...
        if ( target_firmware == "smoothieware") {	
			...
			//MIRKO	
			if (!remove) {
			   url = esp32_address + files_currentPath.replace(primary_sd, "/SD/") + entry.sdname;
			}else{
			...
			//MIRKO	
			if (!remove) {
			   url =  esp32_address + "/SD/" + files_currentPath+ entry.sdname;
			}else{
...
function files_refreshFiles(path, usecache) {
    ...
    if ((current_source==tft_sd) || (current_source==tft_usb)){
     ...
     getById('print_upload_btn').style.display="block";
    }
    if (typeof usecache === 'undefined') usecache = false;
    getById('files_currentPath').innerHTML = files_currentPath;
    files_file_list = [];
    files_status_list = [];
    files_build_display_filelist(false);
    getById('files_list_loader').style.display = "block";
    getById('files_nav_loader').style.display = "block";
    ...
    if (direct_sd && !(target_firmware == "smoothieware" && files_currentPath.startsWith(secondary_sd))) {
		...
		//MIRKO
	    if (!remove) {
	      url = esp32_address  + "/upload?path=" + encodeURI(cmdpath);
	    }else{
	      url = "/upload?path=" + encodeURI(cmdpath);
	    }
        ...
        if (target_firmware == "smoothieware") {
            ...
            if (current_source == "SD:") {
                getById('fileSource').innerHTML="TFT SD";
                if (path.endsWith("/")){
                ...
                getById('fileSource').innerHTML="TFT USB";
                if (path.endsWith("/")){
                    var newpath = path.substring(0, path.length - 1);
                    path= newpath;
                }
                ...
                //Standard M20
                current_source = "/";
                getById('fileSource').innerHTML=translate_text_item("SD Files");
            }
...
function files_is_filename(file_name) {
    ...
    if ((s_name.length == 0) || (s_name.indexOf(":") != -1) || (s_name.indexOf("..") != -1)){
	  answer = false;	
	} 
...
function files_build_display_filelist(displaylist) {
    var content = "";
    getById('files_uploading_msg').style.display = "none";
    if (typeof displaylist == 'undefined') displaylist = true;
    getById('files_list_loader').style.display = "none";
    getById('files_nav_loader').style.display = "none";
    if (!displaylist) {
        getById('files_status_sd_status').style.display = "none";
        getById('files_space_sd_status').style.display = "none";
        getById('files_fileList').innerHTML = "";
        getById('files_fileList').style.display = "none";
        return;
    }
    if (need_up_level()) {
    ...
    getById('files_fileList').style.display = "block";
    getById('files_fileList').innerHTML = content;
    ...
    if (files_status_list.length > 0) {
        if (files_status_list[0].total != "-1") {
            getById('files_sd_status_total').innerHTML = files_status_list[0].total;
            getById('files_sd_status_used').innerHTML = files_status_list[0].used;
            getById('files_sd_status_occupation').value = files_status_list[0].occupation;
            getById('files_sd_status_percent').innerHTML = files_status_list[0].occupation;
            getById('files_space_sd_status').style.display = "table-row";
        } else {
            getById('files_space_sd_status').style.display = "none";
        }
        ...
        if (files_status_list[0].status.toLowerCase() != "ok") {
            getById('files_sd_status_msg').innerHTML = translate_text_item(files_status_list[0].status, true);
            getById('files_status_sd_status').style.display = "table-row";
        } else {
            getById('files_status_sd_status').style.display = "none";
        }
...
function files_select_upload() {
    getById('files_input_file').click();
}

function files_check_if_upload() {
    var canupload = true;
    var files = getById("files_input_file").files;
...
function process_check_sd_presence(answer) {
    ...
    if (direct_sd && !(target_firmware == "smoothieware" && files_currentPath.startsWith(secondary_sd))) {
        if (answer.indexOf("o SD card") > -1) {
            ...
            files_build_display_filelist(false);
            getById('files_sd_status_msg').innerHTML = translate_text_item(files_error_status, true);
            getById('files_status_sd_status').style.display = "table-row";
         }else{ 
		    files_start_upload();
		 }
...
function files_start_upload() {
    ...
    if (!direct_sd || (target_firmware == "smoothieware" && files_currentPath.startsWith(secondary_sd))) {
		 ...
		 //MIRKO	 
		 if (!remove) {
	       url = esp32_address + "/upload_serial";
	     }else{
           url = "/upload_serial";
		 }
    ...
    getById('files_currentUpload_msg').innerHTML = file.name;
    getById('files_uploading_msg').style.display = "block";
    getById('files_navigation_buttons').style.display = "none";
    if (direct_sd && !(target_firmware == "smoothieware" && files_currentPath.startsWith(secondary_sd))) {
    ...
    getById("files_input_file").value = "";
}
...
function FilesUploadProgressDisplay(oEvent) {
    if (oEvent.lengthComputable) {
        ...
        getById('files_prg').value = percentComplete;
        getById('files_percent_upload').innerHTML = percentComplete.toFixed(0);
    } else {
