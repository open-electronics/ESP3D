...
function Set_active_extruder() {
    current_active_extruder = "T" + getById('active_extruder').value;
...
function process_feedRate(msg){
        ...
        getById('feedratecatched').innerHTML = fr;
        if (currentFR==""){
            getById('feedSelectedValue').value=parseInt(fr);
            currentFR=fr;
        }
}
...
function process_flowdRate(msg){
        ...
        getById('flowratecatched').innerHTML = flr;
        if (currentFLR==""){
            getById('flowSelectedValue').value=parseInt(flr);
            currentFLR=flr
        }
}

function on_extruder_length_Change() {
    var value = parseInt(getById('filament_length').value);
...
function on_extruder_velocity_Change() {
    var value = parseInt(getById('extruder_velocity').value);
    if (value < 0.001 || value > 9999 || isNaN(value) || (value === null)) {
        //we could display error but we do not
    }
}
...
function Extrude_cmd(extruder, direction) {
    var filament_length = parseInt(getById('filament_length').value);
    var velocity = parseInt(getById('extruder_velocity').value);
...
function flowInit_cmd() {
    getById('flowSelectedValue').value = 100;
    flowSet_cmd();
}

function flowSet_cmd() {
    var command = "M221 S";
    var value = parseInt(getById('flowSelectedValue').value);
...
function feedInit_cmd() {
    getById('feedSelectedValue').value = 100;
    feedSet_cmd();
}

function feedSet_cmd() {
    var command = "M220 S";
    var value = parseInt(getById('feedSelectedValue').value);
...
function fanOff_cmd() {
    getById('fanSelectedValue').value = 0;
    fanSet_cmd();
}

function fanSet_cmd() {
    var command = "M106 S";
    var fvalue = parseInt(getById('fanSelectedValue').value);
