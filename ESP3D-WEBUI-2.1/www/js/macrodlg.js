...
function showmacrodlg(closefn) {
   var modal = setactiveModal('macrodlg.html',closefn);
    ...
    getById('macrodlg_upload_msg').style.display='none';
    showModal() ;
}
...
function build_filename_selection(index){
    ...
    if(entry.filename.length == 0){
		content += "has-error has-feedback";
	}   
...
function build_dlg_macrolist_line(index){
    ...
    if (entry.class == ''){
	 ...
	 content +="</td>"; 
     getById('macro_line_' + index).innerHTML = content;
}

function macro_filename_OnKeyUp(event, index){
    var item = getById("macro_filename_line_" + index);
    var group = getById("macro_filename_input_line_" + index);
    ...
    if (value.length > 0 ){
		...
		getById("icon_macro_status_line_" + index).style.display='none';
    
	} else{
		
        getById("icon_macro_status_line_" + index).style.display='block';
...
function build_dlg_macrolist_ui(){
    ...
    getById('dlg_macro_list').innerHTML = content;
...
function macrodlgUploadProgressDisplay(oEvent){
    if (oEvent.lengthComputable) {
        ...
        getById('macrodlg_prg').value=percentComplete;
        getById('macrodlg_upload_percent').innerHTML = percentComplete.toFixed(0) ;
        getById('macrodlg_upload_msg').style.display='block';
    }else{
